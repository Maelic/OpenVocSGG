vg_path = "/home/maelic/Documents/OpenVocSGG/all_annotations/vg150_full.json"
...
class RelDataset(Dataset):
    def __init__(self, dataset='COCO', max_samples=1000, split='train', negative=True):
    ...
    def __getitem__(self, idx):
        ...
        for rel in img_data['relationships']:
            ...
            x = max(0, x - 30)
            y = max(0, y - 30)
            w = min(img.width, w + 30)
            h = min(img.height, h + 30)

            cropped_img = self.visualize_box(cropped_img, obj, "2", (0, 0, 255))
            cropped_img = self.visualize_box(cropped_img, sub, "1", (0, 0, 255))
        ...
        return img_id, all_gt, all_cropped_imgs, all_orig_imgs
    ...
    def get_pair_data(self, img, pair, negative=False):
        ...
        img3 = self.visualize_box(img3, pair[0], "1", (255, 0, 0))

        img_cropped = img3.crop((x, y, x+w, y+h))

        return img, img_cropped, img3, (sub_label, obj_label), (pair[0], pair[1])
