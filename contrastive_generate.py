...
def generate_contrastive(
    ...
    **kwargs,
    ...
    generation_mode = GenerationMode.GREEDY_SEARCH #self._get_generation_mode(generation_config, assistant_model)
    ...
    self._prepare_special_tokens(generation_config=generation_config)
...
def greedy_search(
    ...
    **model_kwargs,
    ...
    while True:
        ...
        cache_position = torch.arange(0, input_ids.shape[0], dtype=torch.long, device=input_ids.device)

        model_inputs_new = self.prepare_inputs_for_generation(input_ids, cache_position=cache_position, **model_kwargs_new)
        ...
        if model_kwargs_blackout:
            ...
            model_inputs = self.prepare_inputs_for_generation(input_ids, cache_position=cache_position, **model_kwargs_blackout)
            # forward pass to get next token
